...
func (fq *FilesQuery) QueryContacts() *ContactsQuery {
	query := (&ContactsClient{config: fq.config}).Query()
...
func (fq *FilesQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (fq *FilesQuery) Clone() *FilesQuery {
	...
	return &FilesQuery{
		...
		withContacts: fq.withContacts.Clone(),
...
func (fq *FilesQuery) WithContacts(opts ...func(*ContactsQuery)) *FilesQuery {
	query := (&ContactsClient{config: fq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	fq.withContacts = query
	return fq
}
...
func (fq *FilesQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Files, error) {
	...
	if query := fq.withContacts; query != nil {
		if err := fq.loadContacts(ctx, query, nodes,
			func(n *Files) { n.Edges.Contacts = []*Contacts{} },
			func(n *Files, e *Contacts) { n.Edges.Contacts = append(n.Edges.Contacts, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (fq *FilesQuery) loadContacts(ctx context.Context, query *ContactsQuery, nodes []*Files, init func(*Files), assign func(*Files, *Contacts)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(files.ContactsTable)
		s.Join(joinT).On(s.C(contacts.FieldID), joinT.C(files.ContactsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(files.ContactsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(files.ContactsPrimaryKey[0]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*Files]struct{}{byID[outValue]: {}}
	...
	neighbors, err := withInterceptors[[]*Contacts](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
...
func (fq *FilesQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(files.Table, files.Columns, sqlgraph.NewFieldSpec(files.FieldID, field.TypeInt))
	_spec.From = fq.sql
